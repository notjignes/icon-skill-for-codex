...
- New objects: one centered premium tactile 3D micro-object, soft studio light, believable simplified materials with the selected archive’s roughness and highlight response, softened edges, compact palette. Request no cast/drop/contact/floor shadow. Preserve existing shadows in base edits and archive exports.
...
- People-centric subjects use curated UC character busts. Preserve face family, skin tone, crop, proportions, hair fidelity and material finish; never invent a replacement identity by default.

## Input and reference isolation

The working directory is an output destination, not a reference bank. Bundled benchmark outputs and rejected candidates are diagnostic evidence, not positive generation references; use one as an edit target only when the user explicitly selects it. Use only bundled UC Icons images, a user-attached/named image, explicitly selected Icon Launch archive inputs, and the exact output of the current image call. Do not search home/workspace/generated-image folders or other tasks for an input. In SKU workspaces, do not load SKU entrypoints, Visual Director, historical sheets or taste evidence.
...
When the user names a service rather than an object, decide what visual best identifies that category before looking up an asset. Prefer one product/item: a chimney hood for chimney service, a dedicated upholstery extractor for upholstery cleaning. A fixture and its functional parts count as one semantic item: a basin with its tap is a single plumbing fixture. Add a tool when it clarifies service. For plumbing, preserve the user-selected basin/tap assembly with a clearly readable wrench across its lower foreground; do not simplify it to a detached tap plus a tiny floating wrench. Keep a clear primary mass and a readable cue; subordinate does not mean miniature. Match an approved composition’s relative tool size and overlap. Never add a prop just to decorate or fill space. Preserve an explicitly requested subject. Exact known service aliases still export their approved archive icon.

Internally record the service, chosen primary item, optional secondary item and why it is necessary. Pass the service using `--service` alongside the chosen `--subject`; the generation prompt must retain both. People-centric categories keep their established curated-bust exception.
...
Examples: `water purifier` asks normal versus Native RO; `native RO water purifier` exports the Native asset; `Spa for women --change "remove towel; upholstery beige"` edits the massage bed. `female chef` edits the user-calibrated InstaHelp female open-eyes service master. `office chair`, `gift box`, or `black van` must not become a sofa, truck, or massage-table edit because of color or token overlap.
...
Default new-object view: straight front, near-orthographic. A true level side elevation is also valid when the primary object reads better in profile; never drift into an oblique compromise. Validate the primary object itself: unintended top/underside planes, foreshortening and diagonal recession fail a requested front/side view even if the secondary tool is frontal. Wide/long subjects such as vehicles, sofas, beds and benches use a level side profile. Tall/compact subjects remain front-facing; slight top visibility is allowed when needed for recognition. Base edits preserve the source view. Explicit composition choices override these defaults without changing UC material style or adding a scene. For unusual silhouettes, infer the actual main object first and pass the appropriate supported composition.

Read `references/skeuo-uc-characters.md` only for character edits. It contains canonical eye-state, outfit, treatment and accent guidance. New service characters edit the returned curated base, even with `--new-design`. Open-eyed female service variants now use the InstaHelp master, following the user’s skin/crop calibration; preserve its actual light, face-to-bust ratio and short upper-chest crop. Other gender/eye states retain their canonical bases. An explicitly user-selected curated source overrides default base choice. Use an external person reference for local clothing/treatment cues; preserve the UC crop unless the user explicitly requests its framing. Do not copy a real person's likeness.
...
Before delivery, compare the result with its actual style/base reference at the same canvas size and at 48–96 px display width: primary camera, diffuse-light direction and contrast, material roughness/highlights, and (for characters) skin hue, face-to-bust ratio and lower crop. Check that the tool remains identifiable at small size, not just visible as a thin shape. Category readability alone does not pass the family-consistency check. A reference’s incidental metal part is not permission to change the whole icon’s lighting or finish. If an archive master already solves the request, export it instead of regenerating it.
